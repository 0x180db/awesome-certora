...
function add_contract() {
    ...
    CAMEL_CASED_CONTRACT_NAME="$(tr '[:upper:]' '[:lower:]' <<< ${CONTRACT_NAME:0:1})${CONTRACT_NAME:1}"
    HARNESS_NAME="${CONTRACT_NAME}Harness"
    HARNESS_FILE="$HARNESS_DIR/${HARNESS_NAME}.sol"
    SPEC_FILE="$SPECS_DIR/${CAMEL_CASED_CONTRACT_NAME}.spec"
...
EOF
    ...
    if [ "$FLAG" = "--methods" ]; then
        ...
        forge inspect "$CONTRACT_PATH":"$CONTRACT_NAME" abi --json | jq -r --arg name "$CONTRACT_NAME" '
          .[] |
...
function create_spec_and_conf() {
    ...
    FLATTENED_CONTRACT=$(forge flatten "$CONTRACT_PATH")
    ...
    echo "$FLATTENED_CONTRACT" | solc - --combined-json ast | jq -r '
      .sources[].AST.nodes[]
...
printf "Certora Project Bootstrap — by 0x180db\n"
printf "Spec layout follows the style guide by AlexZoid\n\n"
printf "Usage: $0 <command> [options]\n\n"
printf "Commands:\n"
printf "  ${BOLD}init${NORMAL}\n"
printf "\tScaffold basic Certora project structure.\n\n"

printf "  ${BOLD}add path/to/Contract.sol [--methods]${NORMAL}\n"
printf "\tAdd a base Certora spec and harness for a given contract. Requires Foundry.\n"
printf "\tWith --methods, extract ABI methods into 'methods { }'.\n\n"

printf "  ${BOLD}spec path/to/Contract.sol [--dispatcher]${NORMAL}\n"
printf "\tGenerate verifiable Certora spec and conf files. Requires Solc.\n"
printf "\tWith --dispatcher, adds DISPATCHER summaries for all external calls.\n"
        exit 1
        ;;
esac
